...
from solid_dmft.gw_embedding.tools import calc_Pi_imp_NN
...
def embedding_driver(general_params, solver_params, gw_params, advanced_params):
    ...
    if mpi.is_master_node():
        ...
        ir_mesh_idx_f = ir_kernel.wn_mesh(stats='f',ir_notation=False)
        ir_mesh_idx_b = ir_kernel.wn_mesh(stats='b',ir_notation=False)
        ir_mesh = (2*ir_mesh_idx_f+1)*np.pi/gw_params['beta']
        Sigma_ir = np.zeros((len(ir_mesh_idx_f),
        ...
        Pi_imp_dlr_iw = [None] * sumk.n_inequiv_shells
        Pi_imp_wsIabcd = np.zeros((len(ir_mesh_idx_b),
                                 gw_params['number_of_spins'],
                                 sumk.n_inequiv_shells,
                                 max(gw_params['n_orb']),
                                 max(gw_params['n_orb']),
                                 max(gw_params['n_orb']),
                                 max(gw_params['n_orb'])),dtype=complex)
    ...
    for ish in range(sumk.n_inequiv_shells):
        ...
        if mpi.is_master_node():
            ...
            # calculate impurity polarization function
            if solvers[ish].solver_params['type'] == 'ctseg' and solvers[ish].solver_params['measure_nnt']:
                mpi.report(f'Calculating impurity polarization function for impurity {ish}')
                chi_imp_dlr = fit_gf_dlr(solvers[ish].triqs_solver.results.nn_tau,
                                         eps=gw_params['mesh_dlr_iw_b'].eps,
                                         w_max=gw_params['mesh_dlr_iw_b'].w_max,
                                         symmetrize=True)
                chi_imp_dlr_iw = make_gf_dlr_imfreq(chi_imp_dlr)
                U_dlr_dd_iw = Gf(mesh=chi_imp_dlr_iw.mesh, target_shape=chi_imp_dlr_iw.target_shape)
                norb = gw_params['n_orb'][ish]

                if general_params['h_int_type'][ish] in  ('crpa', 'crpa_density_density'):
                    Uloc_0 = make_gf_imfreq(gw_params['Uloc_dlr'][ish]['up'],1)
                    Uloc = Uloc_0.data[0,:,:,:,:] + gw_params['Vloc'][ish][block]['up']
                    U, Uprime = reduce_4index_to_2index(Uloc)
                    # fill U_dlr_dd_iw with static U
                    U_dlr_dd_iw[0:norb, 0:norb] << U
                    U_dlr_dd_iw[norb:2*norb, norb:2*norb] << U
                    U_dlr_dd_iw[0:norb, norb:2*norb] << Uprime
                    U_dlr_dd_iw[norb:2*norb, 0:norb] << Uprime
                else:
                    Uloc = make_gf_dlr_imfreq(gw_params['Uloc_dlr'][ish]['up'])
                    V, Vprime = reduce_4index_to_2index(gw_params['Vloc'][ish]['up'])
                    for iwn in Uloc.mesh:
                        # get density-density terms and fill U_dlr_dd_iw
                        U, Uprime = reduce_4index_to_2index(Uloc[iwn])
                        U_dlr_dd_iw[iwn][0:norb, 0:norb] = U + V
                        U_dlr_dd_iw[iwn][norb:2*norb, norb:2*norb] = U + V
                        U_dlr_dd_iw[iwn][0:norb, norb:2*norb] = Uprime + Vprime
                        U_dlr_dd_iw[iwn][norb:2*norb, 0:norb] = Uprime + Vprime

                Pi_NN_dlr_iw = calc_Pi_imp_NN(chi_imp_dlr_iw, U_dlr_dd_iw)

                Pi_imp_dlr_iw[ish] = Gf(mesh=Pi_NN_dlr_iw.mesh, target_shape=gw_params['Uloc_dlr'][ish]['up'].target_shape)
                Pi_dlr_iw_ijij = Pi_NN_dlr_iw[0:norb, norb:2*norb]
                # get ijji | iijj term from ijij - antipar term
                Pi_dlr_iw_ijji = Pi_dlr_iw_ijij - Pi_NN_dlr_iw[0:norb, 0:norb]
                for iwn in Pi_imp_dlr_iw[ish].mesh:
                    Pi_imp_dlr_iw[ish][iwn] = construct_Uijkl(Pi_dlr_iw_ijij[iwn], Pi_dlr_iw_ijji[iwn])
                Pi_imp_dlr = make_gf_dlr(Pi_imp_dlr_iw[ish])

                # store Pi_imp_dlr on IR mesh
                iw_mesh_b = MeshImFreq(beta=general_params['beta'], statistic='Boson', n_iw=general_params['n_iw'])
                for iw in range(len(ir_mesh_idx_b)):
                    Pi_imp_wsIabcd[iw,:,ish] = Pi_imp_dlr(iw_mesh_b(ir_mesh_idx_b[iw]))
            ...
            iw_mesh_f = solvers[ish].Sigma_freq.mesh
            # store Sigma and V_HF in sumk basis on IR mesh
            ir_nw_half = len(ir_mesh_idx_f)//2
            for i, (block, gf) in enumerate(Sigma_dlr_sumk):
                ...
                for n in range(ir_nw_half):
                    ...
                    Sigma_ir[iw_pos,i,ish] = gf(iw_mesh_f(ir_mesh_idx_f[iw_pos]))
                    Sigma_ir[iw_neg,i,ish] = gf(iw_mesh_f(ir_mesh_idx_f[iw_pos])).conj()
                for iw in range(len(ir_mesh_idx_f)):
                    Sigma_ir[iw,i,ish] = gf(iw_mesh_f(ir_mesh_idx_f[iw]))
    ...
    if mpi.is_master_node():
        with HDFArchive(general_params['jobname'] + '/' + general_params['seedname'] + '.h5', 'a') as ar:
            ...
            for ish in range(sumk.n_inequiv_shells):
                ...
                if solvers[ish].solver_params['type'] == 'ctseg' and solvers[ish].solver_params['measure_nnt']:
                    ar['DMFT_results/it_{}'.format(iteration)][f'Pi_imp_dlr_iw_{ish}'] = Pi_imp_dlr_iw[ish]
                    ar['DMFT_results/it_{}'.format(iteration)][f'Chi_imp_dlr_{ish}'] = chi_imp_dlr[ish]
        ...
        with HDFArchive(gw_params['h5_file'],'a') as ar:
            ...
            ar[f'downfold_2e/iter{iteration}']['Pi_imp_wsIabcd'] = Pi_imp_wsIabcd
